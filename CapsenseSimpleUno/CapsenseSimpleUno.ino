...
void setup(){
  ...
  delay(100);
  for (int i = 0; i < 8; i++) {
    calibration += readCapacitivePin(capSensePin);
    delay(20);
  }
...
void loop(){
  ...
  //  turn on the LED
  if (readCapacitivePin(capSensePin) > calibration) {
  ...
  if ( (millis() % 500) == 0){
    ...
    Serial.println(readCapacitivePin(capSensePin)-calibration);
  }
}
